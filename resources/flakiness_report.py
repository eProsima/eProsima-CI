...
class FlakyTestsPublisher:
    ...
    def __init__(self, junit_files_dir: str, window_size: int, delete_old_files: bool = True):
        ...
        self._junit_files = FlakyTestsPublisher._find_test_results(junit_files_dir)
    ...
    def publish_report(self, output_file: str):
        """
        Publish the report of the flaky tests found to a markdown file.
        ...
        for test_name, (flip_rate, consecutive_failures, consecutive_passes, failures) in self._flip_rates.items():
            report += f'| {i} | {test_name} | {round(flip_rate, 2)*100} | {consecutive_failures} | {consecutive_passes} | {failures} |\n'
            i += 1
    ...
    @staticmethod
    def _find_test_results(directory):
        """
        ...
        """
        pattern = re.compile(r'^.*test_results.*' + FlakyTestsPublisher._timestamp_rstr + r'\.xml$')
...
if __name__ == "__main__":
    ...
    if args.output_file:
        publisher.publish_report(args.output_file)
